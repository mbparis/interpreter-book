...
var _ = Describe("Lexer", func() {
	...
	Describe("will iterate over source code; not necessarily syntax corect", Ordered, func() {
		input := `
		let five = 5;
		let ten = 10;
		...
		DescribeTable("outputting source code as tokens",
			...
			Entry("when RPAREN", token.TokenType(token.RPAREN), ")"),
			Entry("when SEMICOLON", token.TokenType(token.SEMICOLON), ";"),
			Entry("when BANG", token.TokenType(token.BANG), "!"),
			Entry("when MINUS", token.TokenType(token.MINUS), "-"),
			Entry("when SLASH", token.TokenType(token.SLASH), "/"),
			Entry("when ASTERISK", token.TokenType(token.ASTERISK), "*"),
			Entry("when INT", token.TokenType(token.INT), "5"),
			Entry("when SEMICOLON", token.TokenType(token.SEMICOLON), ";"),
			Entry("when INT", token.TokenType(token.INT), "5"),
			Entry("when LT", token.TokenType(token.LT), "<"),
			Entry("when INT", token.TokenType(token.INT), "10"),
			Entry("when GT", token.TokenType(token.GT), ">"),
			Entry("when INT", token.TokenType(token.INT), "5"),
			Entry("when SEMICOLON", token.TokenType(token.SEMICOLON), ";"),
			Entry("when EOF", token.TokenType(token.EOF), ""),
		)
